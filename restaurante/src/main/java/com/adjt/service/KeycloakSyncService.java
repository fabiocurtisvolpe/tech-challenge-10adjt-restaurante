...
public class KeycloakSyncService {
    ...
    public UUID criarUsuario(Usuario usuario, String senha) {
        try (Keycloak keycloak = getClient()) {
            UserRepresentation user = getUserRepresentation(usuario);
    ...
    public void atualizarUsuario(Usuario usuario) {
        try (Keycloak keycloak = getClient()) {
            List<UserRepresentation> users = keycloak.realm(targetRealm).users().search(usuario.getEmail());
            ...
            if (!users.isEmpty()) {
                ...
                if (!Objects.equals(usuario.getEmail(), user.getEmail())) {
                ...
                LOG.infof("Usuário atualizado no Keycloak: %s", usuario.getEmail());
    ...
    private static @NonNull UserRepresentation getUserRepresentation(Usuario usuario) {
    ...
    private void atribuirRole(Keycloak keycloak, String userId, String roleName) {
        try {
            ...
            LOG.infof("Role %s atribuída ao usuário ID: %s", roleName, userId);
        } catch (Exception e) {
